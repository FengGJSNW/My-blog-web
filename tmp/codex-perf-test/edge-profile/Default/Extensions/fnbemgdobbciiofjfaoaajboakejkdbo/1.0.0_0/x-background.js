...
function detectPrivateMode(cb) {
  ...
  if (isFirefox) {
    ...
    tempMode = localStorage.getItem('SwitchyOmegaMV3.isPrivateMode')
    if (tempMode) {
      tempMode == 'true' ? on() : off()
    } else {
      db = indexedDB.open("SwitchyOmegaMV3-test"), db.onerror = on, db.onsuccess = off
    }
  } else {
    off()
  }
...
detectPrivateMode(function (isPrivateMode) {
  if (isFirefox) {
    localStorage.setItem('SwitchyOmegaMV3.isPrivateMode', isPrivateMode ? 'true' : 'false')
  }
