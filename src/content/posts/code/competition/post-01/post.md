...
```
该解法时间复杂度在O(n^2logn)，大概能解决长度小于1e4的数组（1s内），但是目标是要解决1e6级别的，显然，差了两个数量级，我们要继续优化
...
然后，我们就可以这样快速访问gcd(x, k)的因子：
gcd(x, k) = factor_table[gcd(x,k)][it] (it的范围为gcd(x,k)的因子个数)

当我们再次面对$$gcd(x,y) \mid gcd(x,k)$$，只需这样子:
```cpp
for(auto it : factor_table[gcd(x,k)]) {}
```
就可以遍历所有gcd(x, k)的情况了。
